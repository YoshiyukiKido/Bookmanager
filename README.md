## 書籍管理Webアプリ
このアプリはLaravel13をベースに作られている．ローカルで動かすためには以下のコマンドを実行する必要がある．
...
npm run build
```
これらは，laravel13で必要なコンポーネントをダウンロード，インストールするために行っている．npmはnodejs/JavaScriptのパッケージ管理コマンドだが，Bootstrap, JQueryなどもlaravel13には含まれているため，npmの操作も必要となる．
次に環境の設定を行う必要がある
```
cp .env.example .env
```
